...
/// Port of the iOS `IntervalTimerViewModel` (ViewModels.swift) including its
/// fixes — mirror of [FightTimerController] for interval workouts.
class IntervalTimerController extends ChangeNotifier {
  IntervalTimerController(this.workout)
      : _engine = TimerSessionEngine.interval(workout) {
    final TimerSessionSnapshot s = _engine.snapshot(DateTime.now());
    phase = s.phase;
    timeRemaining = s.timeRemaining;
    currentInterval = s.currentStep;
    _lastSnapshot = s;
  }
  ...
  String phaseText(Translations t) {
    switch (phase) {
      ...
      case TimerPhase.round:
        return '${t.phaseWork} $currentInterval/${workout.intervals}';
  ...
  void updateWorkout(IntervalWorkout w) {
    workout = w;
    reset();
  }
